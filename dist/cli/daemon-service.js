...
function daemonOptions(action, trailing) {
    if (action === "approval-mode") {
        if (trailing.length !== 1) {
            throw new Error("daemon approval-mode requires exactly one of: ask, auto, bypass");
        }
...
async function installedApprovalMode(plistPath) {
    if (!(await exists(plistPath)))
        return null;
    const source = await readFile(plistPath, "utf8");
    const match = source.match(/<key>GATEWAY_FOR_PREMIERE_APPROVAL_MODE<\/key>\s*<string>([^<]+)<\/string>/);
    return match ? resolveApprovalMode(unxml(match[1])) : "ask";
}
async function installedLaunchAgentValue(plistPath, key) {
    if (!(await exists(plistPath)))
        return null;
    const source = await readFile(plistPath, "utf8");
    if (key === "pathValue") {
    ...
    if (!argumentsBlock)
        return null;
    const values = [...argumentsBlock.matchAll(/<string>([^<]+)<\/string>/g)].map((match) => unxml(match[1]));
...
async function runDarwinDaemonService(port, argv, writer, dependencies = {}) {
    ...
    if (action === "install" || action === "approval-mode") {
        if (action === "approval-mode" && !(await exists(plistPath))) {
            throw new Error("Daemon is not installed; run `gateway-for-premiere daemon install --approval-mode MODE`");
        }
        await mkdir(launchAgentsDir, { recursive: true });
        await mkdir(logsDir, { recursive: true });
        const nodePath = (action === "approval-mode"
            ? await installedLaunchAgentValue(plistPath, "nodePath")
            : null) ||
            dependencies.nodePath ||
            process.execPath;
        const entrypoint = (action === "approval-mode"
        ...
        const pathValue = (action === "approval-mode"
...
function windowsVbsValue(value) {
    return value.replaceAll('"', '""');
}
function buildWindowsHiddenLauncher(launcherPath) {
    return [
...
async function runWindowsDaemonService(port, argv, writer, dependencies) {
    ...
    const userDomain = dependencies.windowsUserDomain || process.env.USERDOMAIN || "";
    ...
    const runEntryExists = () => {
        const result = run(spawn, "reg.exe", ["query", WINDOWS_RUN_KEY, "/v", WINDOWS_RUN_VALUE], true);
        return !result.error && result.status === 0;
    };
    const autostartMethod = () => {
        ...
        if (runEntryExists())
            return "startup-registry";
        return null;
    };
    const serviceStatus = async (warnings = [], registered) => {
        const config = await readConfig();
        const pid = await windowsPid(config?.pidPath || pidPath, processRunning);
        const installed = config !== null;
        const autostart = registered !== undefined
            ? registered
            : installed
                ? autostartMethod()
                : null;
    ...
    if (action === "install" || action === "approval-mode") {
        ...
        const nodePath = previousConfig?.nodePath || dependencies.nodePath || process.execPath;
        const entrypoint = previousConfig?.entrypoint || dependencies.entrypoint || process.argv[1];
        if (!entrypoint)
            throw new Error("Could not determine gateway-for-premiere entrypoint");
        ...
        await writeFile(hiddenLauncherPath, buildWindowsHiddenLauncher(launcherPath), "utf8");
        ...
        if (taskRegistered) {
            ...
            if (runEntryExists()) {
                run(spawn, "reg.exe", ["delete", WINDOWS_RUN_KEY, "/v", WINDOWS_RUN_VALUE, "/f"], true);
            }
        }
        else {
            warnings.push(`Could not register the sign-in scheduled task: ${lastTaskFailure}`);
            if (taskExists()) {
                warnings.push(`A scheduled task named "${WINDOWS_DAEMON_TASK}" already exists and could not be updated. It may still start an earlier installation at sign-in.`);
            }
            const registryResult = run(spawn, "reg.exe", [
                "add",
                WINDOWS_RUN_KEY,
                "/v",
                WINDOWS_RUN_VALUE,
            ...
            if (registryResult.error || registryResult.status !== 0) {
                warnings.push(`Could not register the sign-in Run entry: ${failureDetail(registryResult)}`);
                warnings.push("The broker is running now, but it will not start again automatically. Run `gateway-for-premiere daemon start` after each sign-in.");
            }
            else {
                registered = "startup-registry";
                warnings.push(`Registered sign-in autostart under ${WINDOWS_RUN_KEY} instead.`);
            }
        }
        startDetached(config);
        await waitForWindows(pidPath, true, processRunning, delays);
        writer(`${JSON.stringify(await serviceStatus(warnings, registered), null, 2)}\n`);
        return;
    }
    const config = await readConfig();
    if (!config) {
    ...
    if (action === "uninstall") {
        const warnings = [];
        if (taskExists()) {
            const result = run(spawn, "schtasks.exe", ["/Delete", "/TN", WINDOWS_DAEMON_TASK, "/F"], true);
            if (result.error || result.status !== 0) {
                warnings.push(`Could not remove the scheduled task: ${failureDetail(result)}`);
            }
        }
        if (runEntryExists()) {
            const result = run(spawn, "reg.exe", ["delete", WINDOWS_RUN_KEY, "/v", WINDOWS_RUN_VALUE, "/f"], true);
            if (result.error || result.status !== 0) {
                warnings.push(`Could not remove the sign-in Run entry: ${failureDetail(result)}`);
            }
        }
