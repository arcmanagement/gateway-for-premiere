...
function parseGlobal(argv: string[]): {
  ...
  while (index < argv.length && argv[index]?.startsWith("--")) {
    ...
    if (name !== "--port") throw new Error(`Unknown global option: ${name}`);
...
function ensurePositionals(positionals: string[], expected: string[]): void {
  if (
...
async function exportPaths(
  outputFile: string,
  presetFile: string,
): Promise<{ outputFile: string; presetFile: string }> {
...
async function newOutputPath(outputFile: string): Promise<string> {
...
function expectations(
  ...
  const allowOpaqueTransitions = booleanOption(
    options,
    "--allow-opaque-transitions",
  );
  const allowIncompleteRevision = booleanOption(
    options,
    "--allow-incomplete-revision",
  );
  return {
...
function requestIdArgument(
  options: Map<string, string | true>,
): Record<string, string> {
  const value = options.get("--request-id");
  if (value === true) throw new Error("--request-id requires a value");
...
async function responseJson(response: Response): Promise<RpcResponse> {
  ...
  try {
    ...
    throw new Error(
      `Gateway returned non-JSON response (${response.status}): ${text.slice(0, 160)}`,
    );
  }
}
...
async function call(
  ...
  try {
    ...
    throw new Error(
      `${error instanceof Error ? error.message : String(error)}${operationRequestId ? ` (request ${operationRequestId})` : ""}`,
    );
  }
  ...
  if (!response.ok || !value.ok) {
    ...
    throw new Error(
      `${value.error || `Gateway request failed: ${response.status}`}${failedRequestId ? ` (request ${failedRequestId})` : ""}`,
    );
  }
  if (
    operationRequestId &&
...
export async function runCli(
  ...
  if (parsedCommand === "doctor") {
    ...
    try {
      ...
      daemon = JSON.parse(daemonOutput) as Record<string, unknown>;
    } catch (error) {
      daemon = {
    ...
    let health: DoctorHealth | undefined;
  ...
  if (parsedCommand === "plugin" && action === "build") {
    ...
    const distribution = String(
      parsed.options.get("--distribution") || "development",
    );
  ...
  const session =
    typeof parsed.options.get("--session") === "string"
  ...
  if (parsedCommand === "capabilities" && action === undefined) {
    ...
    return output(
      writer,
      await call(fetcher, port, "get_editing_capabilities", {}, session),
    );
  }
  ...
  if (parsedCommand === "project" && action === undefined) {
    ...
    return output(
      writer,
      await call(fetcher, port, "get_active_project", {}, session),
    );
  }
  if (parsedCommand === "project" && action === "recovery") {
    ...
    return output(
      writer,
      await call(fetcher, port, "get_project_recovery", {}, session),
    );
  }
  ...
  if (parsedCommand === "project" && action === "backup") {
    ...
    const outputFile = await newOutputPath(
      required(parsed.options, "--output"),
    );
  ...
  if (
    ...
    if (trackIndex < 0) throw new Error("--track must be >= 0");
    return output(
      writer,
      await call(
        fetcher,
  ...
  if (parsedCommand === "timeline" && action === "trim") {
    ...
    if (startSeconds === undefined && endSeconds === undefined) {
      throw new Error(
        "timeline trim requires --start-seconds or --end-seconds",
      );
    }
    return output(
      writer,
    ...
    return output(
      ...
      await call(
        ...
        "clone_track_item",
        {
          ...
          offsetSeconds: numberOption(parsed.options, "--offset-seconds"),
          videoTrackOffset:
            integerOption(parsed.options, "--video-track-offset", false) ?? 0,
          audioTrackOffset:
            integerOption(parsed.options, "--audio-track-offset", false) ?? 0,
    ...
    return output(
      ...
      await call(
        ...
        "remove_track_item",
        {
          ...
          shiftOverlapping: booleanOption(
            parsed.options,
            "--shift-overlapping",
            false,
          ),
    ...
    return output(
      ...
      await call(
        ...
        "add_video_transition",
        {
          ...
          forceSingleSided: booleanOption(
            parsed.options,
            "--force-single-sided",
            false,
          ),
  ...
  if (
    ...
    const effectName = required(
      parsed.options,
      audio ? "--display-name" : "--match-name",
    );
