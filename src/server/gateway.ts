...
function isPremierePluginOrigin(origin: string | undefined): boolean {
  return origin === "file://";
}

function isOperation(value: unknown): value is PluginOperation {
  return (
    READ_OPERATIONS.has(value as PluginOperation) ||
    MUTATION_OPERATIONS.has(value as PluginOperation)
  );
}
...
function requestId(value: unknown): string {
  ...
  if (
    ...
    throw new Error(
      "requestId must be 8-128 characters using letters, numbers, dot, underscore, colon, or hyphen",
    );
  }
  return value;
}
...
export class GatewayServer {
    ...
    const server = createServer(async (request, response) => {
      ...
      try {
        ...
        if (request.method === "POST" && request.url === "/rpc") {
          const value = (await readJson(request)) as Partial<RpcRequest>;
  ...
  async call(
    ...
    const task = connection.tail.then(() => {
      ...
      if (remainingMs <= 0)
        throw new Error(
          `Premiere request expired in session queue: ${operation}`,
        );
  ...
  private async exportSequenceSafely(
    ...
    if (!extension) throw new Error("Export output must have an extension");
    ...
    try {
      ...
      throw new Error(
        `Could not finalize export without overwriting; staged file kept at ${stagingFile}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  ...
  private dispatch(
    ...
    if (connection.socket.readyState !== WebSocket.OPEN) {
      return Promise.reject(
        new Error("Premiere Plugin connection is not open"),
      );
    }
    const id = randomUUID();
    return new Promise((resolve, reject) => {
      ...
      const timer = timeoutMs
        ? setTimeout(() => {
            this.pending.delete(id);
            reject(
              new Error(`Premiere Plugin request timed out: ${operation}`),
            );
          }, timeoutMs)
        : undefined;
      this.pending.set(id, {
      ...
      try {
        connection.socket.send(
          ...
          (error) => {
            if (error) {
              rejectSend(
                new Error(
                  `Could not send request to Premiere Plugin: ${error.message}`,
                ),
              );
            }
          },
        ...
        rejectSend(
          new Error(
            `Could not send request to Premiere Plugin: ${error instanceof Error ? error.message : String(error)}`,
          ),
        );
      }
    });
  ...
  private selectConnection(requestedSessionId?: string): Connection {
    ...
    if (connections.length > 1)
      throw new Error(
        "Multiple Premiere sessions are connected; pass --session",
      );
    ...
    socket.on("message", (data) => {
      try {
        const message = JSON.parse(data.toString()) as
          PluginHello | PluginResponse | PluginSessionUpdated;
        if (message.type === "hello") {
          ...
          if (!sessionId) throw new Error("Plugin hello is missing sessionId");
          ...
          socket.send(
            JSON.stringify({ type: "connected" } satisfies PluginConnected),
          );
          return;
        }
        ...
        if (message.type === "response") {
          ...
          if (!sessionId || !pending || pending.socket !== socket) return;
          clearTimeout(pending.timer);
          this.pending.delete(message.id);
          if (message.ok) pending.resolve(message.result);
          else
            pending.reject(
              new Error(message.error || "Premiere Plugin request failed"),
            );
        }
      } catch (error) {
        socket.close(
